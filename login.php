...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->num_rows === 1) {
        $stmt->bind_result($hashed, $role); $stmt->fetch();
        if (password_verify($password, $hashed)) {
            $_SESSION['username'] = $username;
            $_SESSION['role']     = $role;

            $stmt = $conn->prepare("INSERT INTO logs (username, action, role) VALUES (?, 'login', ?)");
            $stmt->bind_param("ss", $username, $role); $stmt->execute();

            header("Location: ".($role === 'admin' ? 'admin_dashboard.php':'user_dashboard.php')); exit;
        }
    }
